  ...
  for (const garden of gardens ?? []) {
    try {
      ...
      const templateRows = templates
        ...
        .map((template) => ({ template, dueDate: addDays(new Date(), getCadenceRule(template).intervalDays) }))
      ...
      if (newlyOverdue.length > 0) {
        const { error: overdueError } = await supabase
          ...
          .in("id", newlyOverdue.map((task) => task.id));

        if (overdueError) {
          console.error("mark overdue", garden.id, overdueError.message);
        }
      }
      ...
      failedGardens += 1;
      console.error("runGardenAutomation garden", garden.id, gardenError instanceof Error ? gardenError.message : gardenError);
    }
  }

  return { gardens: gardens?.length ?? 0, createdTasks, createdNotifications, failedGardens };
}
